...
if [ "$1" = "" ] 
then
    echo "./4-update-container-instance.sh [version-number]"
    echo ""
    echo " Example:"
    echo "./4-update-container-instance.sh 10"
    exit 1
fi
export VERSION=$1
...
###############################################################
# 7. Azure Container Instance のインスタンスの更新
#
# Update Azure Container Instance (ACI) with the latest version of the container image
###############################################################
update_container_apps_instance(){
    echo "update_container_apps_instance() is running ..."
    # Update Azure Container App Instance (CUSTOMEJRE)
    echo "----- Update Azure Container App Instance (CUSTOMEJRE) START -----"
    echo name $CUSTOMEJRE_APPLICATION_NAME
    echo image  $DOCKER_REPOSITORY/$CUSTOMEJRE_APP_DOCKER_IMAGE:$VERSION
    date '+%Y/%m/%d %H:%M:%S:%s'
    ...
    date '+%Y/%m/%d %H:%M:%S:%s'
    echo "----- Update Azure Container App Instance (CUSTOMEJRE) END -----"
}
